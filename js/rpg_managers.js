...
DataManager.makeSavefileInfo = function() {
    ...
    info.fileName = $gameActors.actor(3).name();
    info.colorIndex = $gameVariables.value(56);
    info.timestamp  = Date.now();
    info.chapter= $gameVariables.value(97);
    //info.clear=
    return info;
};
...
BattleManager.startBattle = function() {
    this._phase = 'start';
    $gameActors.actor(1).addState(21);
    $gameActors.actor(2).addState(21);
    $gameActors.actor(3).addState(21);
...
BattleManager.selectNextPartyCommand = function() {
    do {
        if (!this.actor() || !this.actor().selectNextPartyCommand()) {
            if($gameActors.actor(2).hp==0 && $gameActors.actor(1).hp==0)
            { 
                indexChange = 3;
            }
            else if($gameActors.actor(1).hp==0)
            { 
                indexChange = 2;
            }
            else{
            ...
            this.changeActor(this._actorIndex + indexChange, 'waiting');
...
BattleManager.selectPreviousCommand = function() {
    do {
        if (!this.actor() || !this.actor().selectPreviousCommand()) {
            ...
            if(this._actorIndex - 1 == 1 && $gameActors.actor(2).hp==0 && $gameActors.actor(1).hp==0)
            { 
                indexChange = 3;
            }
            else if(this._actorIndex - 1 == 1 && $gameActors.actor(2).hp==0)
            { 
                indexChange = 2;
            }
            else if(this._actorIndex - 1 == 0 && $gameActors.actor(1).hp==0)
            {
                indexChange = 2;
            }
            else{
            ...
            this.changeActor(this._actorIndex - indexChange, 'undecided');
...
BattleManager.updateBattleEnd = function() {
    if (this.isBattleTest()) {
        ...
        $gameActors.actor(2).removeState(1);
        $gameActors.actor(3).removeState(1);
        $gameActors.actor(1).removeState(21);
        $gameActors.actor(2).removeState(21);
        $gameActors.actor(3).removeState(21);
        $gameActors.actor(1).removeState(29);
        $gameActors.actor(2).removeState(29);
        $gameActors.actor(3).removeState(29);
    }
    this._phase = null;
};
...
BattleManager.displayDropItems = function() {
    ...
    if (items.length > 0) {
        $gameMessage.newPage();
        var itemCounts = items.reduce(function(counts, item) {
            counts[item.name] = (counts[item.name] || 0) + 1;
            return counts;
        }, {});
        var itemDescriptions = Object.keys(itemCounts).map(function(name) {
            var count = itemCounts[name];
            var pluralizedName = count > 1 ? name + "s" : name;
            return count > 1 ? count + " " + pluralizedName : name;
        });
        var message;
        if (itemDescriptions.length === 1) {
            ...
            var lastItem = itemDescriptions.pop();
            message = itemDescriptions.join(", ") + ", and " + lastItem;
        }
        message = "\\CL     " + message + " found.";
