...
Scene_Map.prototype.updateEncounterEffect = function() {
    if (this._encounterEffectDuration > 0) {
        ...
        $gameScreen.setZoom(zoomX, zoomY, q*1.01);
...
Scene_Battle.prototype.update = function() {
    ...
    if ( this._actorCommandWindow.active&& BattleManager.actor()._hp==0&& BattleManager.actor()._staticHp==0 ){
        this._actorCommandWindow.close();
        $gameParty.members()[BattleManager._actorIndex+1]!=null
        if( BattleManager._actorIndex +1>= $gameParty.size()){
            this.passTurn();
        }
        else
        {
    ...
    if ( this._itemWindow.active && BattleManager.actor()._hp<2 && BattleManager.actor()._staticHp==0 ){
        this.onItemCancel();
        this._itemWindow.active=false;
    }  

    if ( this._skillWindow.active && BattleManager.actor()._hp<2 && BattleManager.actor()._staticHp==0 ){
        this.onSkillCancel();
        this._skillWindow.active=false;
    }  
    if ( this._actorWindow.active && BattleManager.actor()._hp<2 && BattleManager.actor()._staticHp==0 ){
        this.onActorCancel();
        this._actorWindow.active=false;
    }  

    if ( this._enemyWindow.active && BattleManager.actor()._hp<2  && BattleManager.actor()._staticHp==0 ){
        this.onEnemyCancel();
        this._enemyWindow.active=false;
    }  
    if ( this._partyCommandWindow.active && $gameParty.isAllDead()){
        BattleManager.endBattle();
    }

    if ($gameSwitches.value(88)) {
        $gameVariables.setValue(57,this._actorWindow.index()+1);
    } else if ($gameVariables.value(57) > 0) {
        $gameVariables.setValue(57,0);
    }

    if($gameSwitches.value(74))
    {
        this._logWindow.width=0;

    }
    else if (this._logWindow.width==0 && !$gameSwitches.value(74))
    {
        this._logWindow.width=Graphics.boxWidth*.7;
    }
};


Scene_Battle.prototype.passTurn = function() {
    this.forceSkillOnCurrentActor(3);
};

Scene_Battle.prototype.forceSkillOnCurrentActor = function(skillId) {
...
Scene_Battle.prototype.terminate = function() {
    Scene_Base.prototype.terminate.call(this);
    $gameActors.actor(1).removeState(21);
    $gameActors.actor(2).removeState(21);
    $gameActors.actor(3).removeState(21);    
...
Scene_Battle.prototype.createHelpWindow = function() {
    //    this._helpWindow = new Window_Help(0, wy, Graphics.boxWidth*.4, wh*1.2);
    this._helpWindow = new Window_Help();
    this._helpWindow.visible = false;
    this._helpWindow.width= 816*.5;
...
Scene_Battle.prototype.createSkillWindow = function() {
    ...
    this._skillWindow = new Window_BattleSkill(10, wy, 816*.5, wh*1.2);
    this._skillWindow.setHelpWindow(this._helpWindow);
    this._skillWindow.setHandler('ok',     this.onSkillOk.bind(this));
    this._skillWindow.setHandler('cancel', this.onSkillCancel.bind(this));
    //    this._skillWindow.y=816*.2;
...
Scene_Battle.prototype.createItemWindow = function() {
    var wy = this._helpWindow.y + this._helpWindow.height;
    var wh = this._statusWindow.y - wy;
    this._itemWindow = new Window_BattleItem(10, 90, 816*.5, wh*.52);
    this._itemWindow.setHelpWindow(this._helpWindow);
    this._itemWindow.setHandler('ok',     this.onItemOk.bind(this));
    this._itemWindow.setHandler('cancel', this.onItemCancel.bind(this));
    this.addWindow(this._itemWindow);
    ...
    if($gameSwitches.value(181)){
        ...
        // Deactivate the category window so the key items selection stops flashing
        this._categoryWindow.deactivate();

        // Activate the item window so you can interact with it
        this._itemWindow.activate();

        // Update the help window with the description of the first key item
        // Length of all the items the party has (items, weapons, armour, keyitems)
        var len = $gameParty.allItems().length;

        // Select the category for the call to includes() below
        this._itemWindow.setCategory('keyItem');

        // For each item in the party item list
        for (var i = 0; i < len; ++i) {
            // Get the current item
            var item = $gameParty.allItems()[i];

            // If the item is a keyItem
            if (this._itemWindow.includes(item)) {
                // Set the help window to display the item description
...
Scene_Battle.prototype.selectNextPartyCommand = function() {
    BattleManager.selectNextPartyCommand();
    this.changeInputWindow();
};
...
Scene_Battle.prototype.onEnemyOk = function() {
    this._helpWindow.width=Graphics.boxWidth*.5;
    $gameSwitches.setValue(87, false);
    var action = BattleManager.inputtingAction();
    action.setTarget(this._enemyWindow.enemyIndex());

    $gameVariables.setValue(60+action._subjectActorId, action._targetIndex+1);
