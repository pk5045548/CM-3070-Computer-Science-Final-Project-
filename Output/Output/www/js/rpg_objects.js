...
Game_BattlerBase.prototype.revive = function() {
    if (this._hp === 0) {
        this._hp = 1;
        this._staticHp=1;
        Game_Battler.prototype.addStateEffects('addState');
    }
};
...
Game_BattlerBase.prototype.paySkillCost = function(skill) {
    this._staticMp -= this.skillMpCost(skill);
...
Game_Unit.prototype.smoothTarget = function(index) {
    ...
    var member1 = this.members()[index];  
    var redirectedTarget = null;
    var allMembers = this.members(); 
    var aliveMembers = this.aliveMembers(); 

    var actionUserId = BattleManager._subject.actorId ? BattleManager._subject.actorId() : BattleManager._subject.enemyId();

    if (member1 && !member1.isAlive()) {
        if (aliveMembers.length > 0) {
            redirectedTarget = aliveMembers[Math.floor(Math.random() * aliveMembers.length)];
            index = allMembers.indexOf(redirectedTarget);  
            $gameVariables.setValue(60 + actionUserId, index + 1);  
        }
    }

    // Return the original target if alive, otherwise return the random redirected target
    return member1 && member1.isAlive() ? member1 : redirectedTarget;
};
