...
Window_SkillList.prototype.drawMpCost = function(skill, wx, wy, dw) {
    ...
    var text = fmt.format("x " + Yanfly.Util.toGroup(this._actor.skillMpCost(skill)),
                          TextManager.mpA);
    ...
    this.resetFontSettings();
    if (Yanfly.Icon.Mp > 0) {
        var iw = wx + dw - Window_Base._iconWidth*2.1;
        this.drawIcon(Yanfly.Icon.Mp, iw-2, wy + 2);
        dw -= Window_Base._iconWidth + 2;
    }


//    if(this._actor.isStateAffected(skill.id))
//    {
//        this.drawText(this._actor._stateTurns[skill.id], dw/2+16, wy, dw, 'right');
//        this.drawIcon(28, dw/2+16, wy + 2);
//    }
    return returnWidth;
};
