    ...
    if (!this.isBusy() && !this.updateEvent()) {
        switch (this._phase) {
            ...
            case 'actionTargetList':
                ...
                break;
            case 'turnEnd':
                $gameSwitches.setValue(163, false);
                this.updateTurnEnd();
                break;
            case 'battleEnd':
                $gameSwitches.setValue(163, false);
...
Sprite_Enemy.prototype.addVisualSelectWindow = function() {
    if (!this._visualSelect || this._addedVisualSelect) return;
...
Game_Battler.prototype.addState = function(stateId) {
    ...
    if (this instanceof Game_Actor) {
        if (this.actorId() === 2) {
            if (stateId === 9) {
                this._stateTurns[stateId] = 3;
            }
        }
    }

    if (this.canAddStateFreeTurn(stateId)) {
        this.setStateFreeTurn(stateId);
    }
};
...
var selectVisual = false;
var troopChecks = 0

Window_EnemyVisualSelect.prototype.updateWindowSize = function() {

    if (!this._battler || !$gameSwitches.value(87)) 
    {
        troopChecks = 0;
        selectVisual = false;
        return; // Ensure _battler exists
    }
    else{
        var troopSize = $gameTroop.members().length;
        ...
        var height = this._battler.spriteHeight()*1.2 + this.standardPadding() * 2;
...
Window_EnemyVisualSelect.prototype.updateWindowPosition = function() {
    ...
    this.y = -1 * this.height + this.standardPadding() + 20;  // Add 20 pixels to move the window down
...
Window_EnemyVisualSelect.prototype.refresh = function() {
    ...
    $gameVariables.setValue(86, text);
    this.height+=16;
    var wy = -6;
    this.changeTextColor(this.powerDownColor());
    this.contents.outlineColor="rgba(0, 0, 0, 1)"   
    if ($gameSwitches.value(168)) {
        this.drawText(text, 0, 372, this.contents.width, 'center');
    } else {
