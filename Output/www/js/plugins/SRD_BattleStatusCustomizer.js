...
(function(_) {
    ...
    Window_BattleStatusUpgrade.prototype.updateMotion = function() {
        ...
        if($gameSwitches.value(99)){
            if(this._actorIndex == 0){
                ...
                if(Math.abs(this._offsetY) < Math.abs(eval(_.selectYMax))) {
                    this._offsetY += eval(_.selectY)*2;
                }
                if(Math.abs(this._offsetX) < Math.abs(eval(_.selectXMax))) {
                    this._offsetX += eval(_.selectX)*2;
                }
            } 
        }
        else if($gameSwitches.value(98) ){
        ...
        else if($gameSwitches.value(97)){
            if(this._actorIndex == 2){
                if(_.selectHigh && this._index === -1) {
                    this.active = true;
                    this.select(0);
                }
        ...
        else if(!$gameSwitches.value(99) && !$gameSwitches.value(98) && !$gameSwitches.value(97) ){
            if(this._actorIndex === $gameVariables.value(57)-1) {
                ...
                if(this._actorIndex === this._statusWindow.index() && $gameVariables.value(57)==0) {
                if(_.inputHigh && this._index === -1) {
                    this.active = true;
                    this.select(0);
                }
    ...
    Window_BattleStatusUpgrade.prototype.drawAllTexts = function() {
        ...
        for(let i = 0; i <= this._texts.length; i++) {
            ...
            if(info) {
                ...
                this.drawText(eval(info.value), eval(info.x), eval(info.y)*1.2, eval(info.width), info.align);
        ...
        for(let i = 0; i <= this._gauges.length; i++) {
            ...
            if(info) {
                if(info.absorb) {
                    ...
                    this.drawBasicGauge1(eval(info.text), eval(info.x), eval(info.y), eval(info.width), eval(info.height), eval(info.cur), eval(info.max), eval(info.color1), eval(info.color2), eval(info.back), eval(info.cm));
                    this._checkForRefresh.push([eval(info.cur), info.cur]);
                }
            }
        }
    ...
    Window_BattleStatusUpgrade.prototype.drawBasicGauge = function(text, x, y, width, height, cur, max, color1, color2, colorBack, cm) {
        ...
        const pad = 4;
        //	this.drawGaugeUpgrade(x, y, width, height, (cur/max), this.normalColor(), this.normalColor(), colorBack);
        this.changeTextColor(this.systemColor());
        this.contents.fontSize = size;
        this.contents.fontFace = _.gaugeFont;
        this.contents.fontItalic = _.gaugeItalic;
        this.contents.textColor = this.systemColor(); 
        ...
        {
            this.drawIcon(12, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)*3.5,44);
        }
        else if($gameSwitches.value(71))
        {
            this.drawIcon(13, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)*3.5,44);
        }
        else if($gameSwitches.value(72))
        {
            this.drawIcon(14, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)*3.5,44);
        }
        else{
            this.drawIcon(15, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)*3.5,44);
        }

        if (actor._staticHp==0 && actor) {
            if(cm) this.drawCurrentAndMax(cur, max, x - pad-2, (y + pad + 1)*3.5, width, this.deathColor(), this.deathColor());
        } else {
            if(cm) this.drawCurrentAndMax(cur, max, x - pad-2, (y + pad + 1)*3.5, width, this.normalColor(), this.normalColor());
        }
    };

    Window_BattleStatusUpgrade.prototype.drawBasicGauge1 = function(text, x, y, width, height, cur, max, color1, color2, colorBack, cm) {
//        const actor = this._actor;
//        width = width || 186;
//        x += eval(_.gaugeX);
//        y += eval(_.gaugeY);
//        const size = eval(_.gaugeFontSize);
//        const pad = 4;
//        //	this.drawGaugeUpgrade(x, y, width, height, (cur/max), this.tpGaugeColor2(), this.tpGaugeColor2(),colorBack);
//        this.changeTextColor(this.systemColor());
//        this.contents.fontSize = size;
//        this.contents.fontFace = _.gaugeFont;
//        this.contents.fontItalic = _.gaugeItalic;
...
//        if($gameSwitches.value(70))
//        {
//            this.drawIcon(28, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)+5,44);
//        }
//        else if($gameSwitches.value(71))
//        {
//            this.drawIcon(29, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)+5,44);
//        }
//        else if($gameSwitches.value(72))
//        {
//            this.drawIcon(30, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)+5,44);
//        }
//        else{
//            this.drawIcon(31, eval(_.gaugeTextX)-5, eval(_.gaugeTextY)+5,44);
//        }
//        if (actor.mp==0 && actor!=null) {
//            if(cm) this.drawCurrentAndMax(cur, max, x - pad-2, y + pad + 1, width, this.crisisColor(), this.crisisColor());
//        } else {
//            if(cm) this.drawCurrentAndMax(cur, max, x - pad-2, y + pad + 1, width, this.tpGaugeColor2(), this.tpGaugeColor2());
//        }
    ...
    Window_BattleStatusUpgrade.prototype.drawGaugeUpgrade = function(x, y, width, height, rate, color1, color2, colorBack) {
        ...
        if(!colorBack.match(/clear/)) {
            this.contents.fillRect(x - 1, gaugeY - 1, width*.8 + 2, height + 2, "#000000");
            this.contents.fillRect(x, gaugeY, width*.8, height, colorBack);
        }
        if(rate === 0) return;
    ...
    Window_BattleStatusUpgrade.prototype.drawIcons = function() {
        ...
        const boxWidth = this.contentsWidth();
        const boxHeight = this.contentsHeight();
        this.drawActorIconsScale(actor, eval(_.iconX), eval(_.iconY)-35, eval(_.iconWidth), eval(_.iconScale));
    };
