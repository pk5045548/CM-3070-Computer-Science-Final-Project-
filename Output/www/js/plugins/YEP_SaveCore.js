...
Window_SaveInfo.prototype.drawGameTitle = function(dy) {
    ...
    this.changeTextColor(this.textColor(DataManager.loadSavefileInfo(this.savefileId()).colorIndex));
    this.drawText("Act " + DataManager.loadSavefileInfo(this.savefileId()).chapter, 0, dy, this.contents.width, 'center');
...
Window_SaveInfo.prototype.drawPartyGraphics = function(dy) {
    ...
    var length = this._info.faces.length;
...
Window_SaveInfo.prototype.drawPartyLevels = function(dy) {
    ...
    for (var i = 0; i < length; ++i) {
        ...
        if (member) {
            ...
            this.changeTextColor(this.textColor(DataManager.loadSavefileInfo(this.savefileId()).colorIndex));
            this.drawText(text, dx + dif, dy, 'center');
        }
        dx += dw
    }
...
Window_SaveInfo.prototype.drawVariable = function(id, dx, dy, dw) {
    ...
    if (varName.length > 0) {
        ...
        dy += diff;
        this.drawText(varName, dx, dy, dw, 'left');
        dy -= diff;
        this._drawData = false;
        this.drawText(text, dx, dy, dw, 'right');
    } else {
...
Scene_File.prototype.onSaveSuccess = function() {
    ...
    if ($gameVariables.value(98, 3)) 
    {
        $gameParty.loseItem($dataItems[7], 1);
        this.popScene();
    } else {
        this._listWindow.refresh();
